...
public abstract class SRBaseInfoView extends XRelativeLayout {
    ...
    private SRCarConditionCardView mCarConditionCardView;
    ...
    protected SRMapCardView mMapCardView;
    private SRMediaCardView mMediaCardView;
    private SROdoMeterCardView mOdoMeterCardView;
    private SRPowerConsumptionCardView mPowerConsumptionCardView;
    ...
    public void init(Context context) {
        this.mSRContext = context;
        LayoutInflater.from(getContext()).inflate(getLayout(), this);
        initAnimator();
        this.mRecyclerView = (XRecyclerView) findViewById(R.id.info_list);
        this.mMapCardView = (SRMapCardView) findViewById(R.id.info_map);
        this.mMediaCardView = (SRMediaCardView) findViewById(R.id.info_media);
        this.mCarConditionCardView = (SRCarConditionCardView) findViewById(R.id.info_car_condition);
        this.mOdoMeterCardView = (SROdoMeterCardView) findViewById(R.id.info_power_odo_meter);
        this.mPowerConsumptionCardView = (SRPowerConsumptionCardView) findViewById(R.id.info_power_consumption);
        ...
        cardPickerLayoutManager.setOnSelectedViewListener(new CardPickerLayoutManager.OnSelectedViewListener() { // from class: com.xiaopeng.instrument.widget.sr.SRBaseInfoView.1
            ...
            public void onSelectedView(View view, int i) {
                int cycleNum = SRBaseInfoView.this.cardListAdapter.getCycleNum() / 2;
                int size = SRBaseInfoView.this.mInfoContainBean.getInfoBeanList().size();
                XILog.i(SRBaseInfoView.this.TAG, "onSelectedView() pos = " + i + " size = " + size + " name = " + SRBaseInfoView.this.mInfoContainBean.getInfoBeanList().get(i % size).getName() + " area = " + SRBaseInfoView.this.getPosition());
                if (i < size * cycleNum || i >= (cycleNum + 1) * size) {
                    SRBaseInfoView.this.cardPickerLayoutManager.scrollToPositionForCycle(i, size);
                }
            }
        });
    }
